...
from hacks_automation import __version__, constants
from invoke import task

basicConfig(level="INFO", format=constants.LOG_FORMAT)
LOG = getLogger("hacks_automation.invoke")
...
IMAGE = "jonzeolla/hacks_automation"
...
def lint(_c, debug=False):
    ...
    environment["SUPPRESS_POSSUM"] = True
    working_dir = "/super-linter/"
    environment["DEFAULT_WORKSPACE"] = working_dir
...
def update(_c, debug=False):
    """Update the core components of HACKS Automation"""
    if debug:
        getLogger().setLevel("DEBUG")
...
def release(_c, debug=False):
    """Make a new release of HACKS Automation"""
    if debug:
        getLogger().setLevel("DEBUG")
